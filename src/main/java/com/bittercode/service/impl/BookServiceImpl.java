...
public class BookServiceImpl implements BookService {
    ...
    private static final String addBookQuery = "INSERT INTO " + BooksDBConstants.TABLE_BOOK + " (barcode, name, author, price, quantity, image_path) VALUES(?,?,?,?,?,?)";
...
//                int bPrice = rs.getInt(4);
                ...
                book = new Book(bCode, bName, bAuthor, bPrice, bQty, imagePath);
            }
...
//                int bPrice = rs.getInt(4);
    ...
    public List<Book> getBooksByCommaSeperatedBookIds(String commaSeperatedBookIds) throws StoreException {
        ...
        Connection con = DBUtil.getConnection();
        try {
            String formattedBookIds = "'" + commaSeperatedBookIds.replaceAll(",", "','") + "'";
...
//                    BooksDBConstants.COLUMN_BARCODE + " IN ( " + commaSeperatedBookIds + " )";
            String getBooksByCommaSeperatedBookIdsQuery = "SELECT * FROM " + BooksDBConstants.TABLE_BOOK
                    + " WHERE " + BooksDBConstants.COLUMN_BARCODE + " IN (" + formattedBookIds + ")";
            System.out.println("BookServiceImpl: SQL=" + getBooksByCommaSeperatedBookIdsQuery);
...
//                int bPrice = rs.getInt(4);
                ...
                Book book = new Book(bCode, bName, bAuthor, bPrice, bQty, imagePath);
                books.add(book);
                System.out.println("BookServiceImpl: Found book - " + bCode);
            }
            System.out.println("BookServiceImpl: Found " + books.size() + " books");
        } catch (SQLException e) {
            System.err.println("BookServiceImpl: SQL Error - " + e.getMessage());
            e.printStackTrace();
